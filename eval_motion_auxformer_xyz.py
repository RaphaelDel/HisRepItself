...
def load_model(cfg: DictConfig, device: torch.device, logger):
    ...
    if ckpt_path is None:
        raise ValueError("Set eval.checkpoint in config for AuxFormer xyz evaluation.")
    ...
    logger.info("Loaded AuxFormer checkpoint from %s (epoch=%s)", ckpt_path, ckpt.get("epoch", "n/a"))
    return model
...
def eval_loader(
    ...
    for batch_idx, batch in enumerate(tqdm(dataloader)):
        ...
        x_exp = rot_gt[:, :obs_len]
        y_exp = rot_gt[:, obs_len : obs_len + pred_len]

        x_xyz = expmap_to_xyz_seq(x_exp, use_clean_fk=use_clean_fk)
        y_xyz = expmap_to_xyz_seq(y_exp, use_clean_fk=use_clean_fk)
        x_xyz_norm = _normalize_pose_cpu(x_xyz.detach().cpu()).to(device)
        jepa_pose_past = None
        if use_jepa_conditioning:
            if jepa_use_vel_ae:
                x_vel_exp = expmap_velocity(x_exp)
                jepa_pose_past = torch.cat([x_exp, x_vel_exp], dim=-1)
            else:
                jepa_pose_past = x_exp

        pred_norm = model.predict_with_prior(
            x_xyz_norm,
            future_frames=pred_len,
            jepa_pose_past=jepa_pose_past,
        )
        pred_xyz = _denormalize_pose_cpu(pred_norm.detach().cpu()).to(device)

        mae_t = torch.mean(torch.abs(pred_xyz - y_xyz), dim=(0, 2, 3))
        mae_xyz_sum += mae_t.detach().cpu().double() * pred_xyz.size(0)

        pred_xyz_m = pred_xyz * scale
        y_xyz_m = y_xyz * scale
        mpjpe_t = torch.norm(pred_xyz_m - y_xyz_m, dim=-1).mean(dim=2)
        mpjpe_sum += mpjpe_t.detach().cpu().double().sum(dim=0)
        n_samples += pred_xyz.size(0)
